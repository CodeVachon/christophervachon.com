...
component output="false" displayname="blog"  {
	...
	public void function before( required struct rc ) {
		...
		RC.template.addFile("cmvVirt.min.css");
		RC.template.addFile("cmvVirt.js");
	} // close before
	...
	public void function endView( required struct rc ) {
		...
		if (len(RC.article.getURI()) > 0) {
			...
			RC.pageTitle = RC.article.getTitle() & " | " & dateFormat(RC.article.getPublicationDate(),"mmm yyyy") & " | " & RC.template.getSiteName();
	...
	public void function search( required struct rc ) {
		...
		if (structKeyExists(RC,"search_for")) {
			...
			RC.template.addPageCrumb("Search","/blog/search");
		}
	} // close search
}
